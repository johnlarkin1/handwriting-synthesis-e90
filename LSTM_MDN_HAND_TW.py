...
def make_heat_plot(epoch, loss, query_data, xrng, yrng, xg, pred, i):
    ...
    plt.title(titlestr)
    plt.savefig('LSTM Query Heat Map ' + str(i) + '.pdf')
...
def main():
    ...
    our_train_data = data.data[0]
    our_valid_data = data.valid_data[0]
    our_query_data = data.valid_data[500]
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % update_every == 0:
            ...
            l, pred = query_model.run_epoch(session, return_predictions=True, query=False)
